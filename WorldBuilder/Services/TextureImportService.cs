...
namespace WorldBuilder.Services {
    ...
    public class TextureImportService {
        ...
        private uint[] GetExistingRenderSurfaceIds() {
            try { return _project.DatReaderWriter.GetAllIdsOfType<RenderSurface>().ToArray(); }
            catch (Exception ex) {
        ...
        private uint[] GetExistingSurfaceTextureIds() {
            try { return _project.DatReaderWriter.GetAllIdsOfType<SurfaceTexture>().ToArray(); }
            catch (Exception ex) {
        ...
        private uint[] GetExistingSurfaceIds() {
            try { return _project.DatReaderWriter.GetAllIdsOfType<Surface>().ToArray(); }
            catch (Exception ex) {
        ...
        public static RenderSurface CreateRenderSurface(uint gid, byte[] bgraData, int width, int height) {
            return new RenderSurface {
                ...
                Width = width,
                Height = height,
                Format = (uint)DatPixelFormat.PFID_A8R8G8B8,
                SourceData = bgraData
            };
        }
        ...
        private void WriteRenderSurfaceReplacementsToDats(IDatReaderWriter writer, int? iteration) {
            ...
            foreach (var entry in replacements) {
                ...
                try {
                    ...
                    if (originalRs.FormatEnum != DatPixelFormat.PFID_A8R8G8B8) {
