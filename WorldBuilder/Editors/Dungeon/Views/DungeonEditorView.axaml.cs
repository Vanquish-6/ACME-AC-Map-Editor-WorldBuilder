    ...
    public partial class DungeonEditorView : UserControl {
        ...
        private void OnTopLevelKeyDown(object? sender, KeyEventArgs e) {
            ...
            if (e.Key == Key.F && !e.KeyModifiers.HasFlag(KeyModifiers.Control)) {
                _viewModel.FocusSelection();
                e.Handled = true;
                return;
            }

            if (_viewModel.IsPlacementMode &&
                e.Key is Key.R or Key.E or Key.Q or Key.Enter or Key.Return or Key.Space) {
                _viewModel.HandleKeyDown(e);
                e.Handled = true;
                return;
            }
        ...
        private void OnDragOver(object? sender, DragEventArgs e) {
            if (EditorDragFormats.TryGetPrefab(e.Data, out _) || EditorDragFormats.TryGetObject(e.Data, out _, out _, out _)) {
        ...
        private void OnDrop(object? sender, DragEventArgs e) {
            HideGhosts();
            if (EditorDragFormats.TryGetPrefab(e.Data, out var signature)) {
                _viewModel?.PlaceDroppedPrefab(signature);
                return;
            }
            if (EditorDragFormats.TryGetObject(e.Data, out var objId, out var isSetup, out var wcid)) {
                _viewModel?.PlaceDroppedObject(objId, isSetup, wcid);
                return;
            }
            var id = e.Data.Get("DockablePanelId") as string;
