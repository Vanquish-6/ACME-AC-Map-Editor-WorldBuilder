...
namespace WorldBuilder.Editors.Dungeon {
    ...
    public class DungeonEditingContext {
        ...
        public float GridSnapSize { get; set; } = 10f;
        public float NudgeStep { get; set; } = 10f;

        /// <summary>Editor-only hidden rooms (not written to DAT).</summary>
        public HashSet<ushort> HiddenCells { get; } = new();
        /// <summary>Editor-only locked rooms — selectable but not movable.</summary>
        public HashSet<ushort> LockedCells { get; } = new();
        /// <summary>Named groups of cell numbers for the outliner.</summary>
        public Dictionary<string, HashSet<ushort>> CellGroups { get; } = new(StringComparer.OrdinalIgnoreCase);

        public bool IsCellHidden(ushort cellNum) => HiddenCells.Contains(cellNum);
        public bool IsCellLocked(ushort cellNum) => LockedCells.Contains(cellNum);

        public bool SelectionIsLocked() {
            if (HasSelectedObject) return IsCellLocked(SelectedObjCellNum);
            foreach (var c in SelectedCells) {
                if (IsCellLocked((ushort)(c.CellId & 0xFFFF))) return true;
            }
            return false;
        }
        ...
        public event Action<List<ushort>>? CellsAdded;
        ...
        public void NotifyCellsAdded(List<ushort> cellNums) => CellsAdded?.Invoke(cellNums);
        ...
        public List<ushort> GetSurfacesForRoom(RoomEntry room) {
            ...
            try {
                var lbiIds = Dats.GetAllIdsOfType<LandBlockInfo>().Take(500).ToArray();
                foreach (var lbiId in lbiIds) {
                    ...
                    for (uint i = 0; i < lbi.NumCells && i < 50; i++) {
                        ...
                        if (ec.EnvironmentId == room.EnvironmentId && ec.CellStructure == room.CellStructureIndex && ec.Surfaces.Count > 0) {
                            room.DefaultSurfaces = ec.Surfaces.Select(s => (ushort)s).ToList();
                            return room.DefaultSurfaces;
                        }
                    }
                }
        ...
        /// </summary>
        public List<EnvCell> BuildPrefabEnvCells(
            ...
            for (int i = 0; i < prefab.Cells.Count; i++) {
                ...
                envCell.Surfaces.AddRange(pc.Surfaces.Select(s => (uint)s));
                result.Add(envCell);
            }
