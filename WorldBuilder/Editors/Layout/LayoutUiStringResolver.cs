...
namespace WorldBuilder.Editors.Layout {
    ...
    public static class LayoutUiStringResolver {
        ...
        public static string? TryGetElementCaption(IDatReaderWriter? dats, ElementDesc element,
        ...
        public static uint? TryResolveEffectiveType(IDatReaderWriter? dats, ElementDesc element, uint containingLayoutId = 0) {
            if (dats == null) return null;
        ...
        static string? TryResolveCaptionOnElementOnly(IDatReaderWriter dats, ElementDesc element,
        ...
        public static string? TryGetCaptionFromStringInfoProperties(IDatReaderWriter dats, ElementDesc element,
        ...
        public static bool TryResolveStringInfoText(IDatReaderWriter dats, StringInfo info,
        ...
        public static bool TryResolveMessageText(IDatReaderWriter dats, ElementDesc element, StateDesc state,
        ...
        /// </summary>
        static IEnumerable<uint> EffectiveStringTableScanOrder(IDatReaderWriter dats, IReadOnlyList<uint>? cachedIds) {
        ...
        static uint? TryStringTableDidWhenStringIdMatches(StateDesc? state, uint messageId) {
            ...
            foreach (var p in state.Properties.Values) {
                if (p is not StringInfoBaseProperty sip)
                    continue;
                var tableDid = sip.Value.TableId;
        ...
        static bool TryLookupRow(IDatReaderWriter dats, uint stringTableId, uint stringKey, out string? text) {
            text = null;
            var visited = new HashSet<uint>();
            return TryLookupRowChain(dats, stringTableId, stringKey, visited, out text);
        }

        static bool TryLookupRowChain(IDatReaderWriter dats, uint stringTableId, uint stringKey,
            HashSet<uint> visitedTables, out string? text) {
            text = null;
            if (!visitedTables.Add(stringTableId))
                return false;
            ...
            foreach (var key in EnumerateStringRowKeys(stringKey)) {
                ...
                var parentTableId = row.DataId;
                if (parentTableId != 0 &&
                    TryLookupRowChain(dats, parentTableId, stringKey, visitedTables, out text) &&
                    !string.IsNullOrWhiteSpace(text))
                    return true;
            }
        ...
        public static string? FormatStringTableRow(StringTableString row) {
            ...
            foreach (var line in row.Strings) {
                ...
                if (StringTableMetaLanguageRenderer.ContainsMetaLanguage(v))
                    v = StringTableMetaLanguageRenderer.RenderForPreview(v);
        ...
        public static string BuildMediaSummary(StateDesc state, IDatReaderWriter? dats, ElementDesc owner,
            IReadOnlyList<uint>? stringTableIdsInLocal = null) {
