...
namespace WorldBuilder.Editors.Landscape.WorldGen {
    ...
    public static class BuildingAnalyzer {
        ...
        /// <summary>
        /// Building models for the object browser: Setup DIDs (0x02xxxxxx) that reach real <see cref="EnvCell"/> records
        /// from the building portal graph (see <see cref="BuildingProfile.CellCount"/>). GfxObj mesh shells are excluded.
        ...
        /// <summary>Placeable buildings use Setup models (0x02xxxxxx). GfxObj (0x01) IDs are mesh shells, not objects.</summary>
        private static bool IsSetupModelId(uint id) => (id & 0xFF000000) == 0x02000000;

        /// <summary>
        /// Catalog for object browser: Setup models with real <see cref="EnvCell"/> interiors + successful blueprint extract.
        /// Drops GfxObj mesh shells, paired landblock modules, and known fragment IDs. Never uses “profile-only” fallback
        ...
        private static List<uint> AnalyzeEditorComplete(IDatReaderWriter dats) {
            ...
            bool AllowedId(BuildingProfile p) =>
                IsSetupModelId(p.ModelId) && !KnownStructuralPieceModelIds.Contains(p.ModelId);
            ...
            // Tier 1: substantial interiors, not villa halves, Setup models only, blueprint must match.
            var tier1 = profiles
                .Where(AllowedId)
            ...
            if (validated.Count == 0) {
                // Tier 2: still exclude paired halves / denylist / GfxObj shells; allow 4 cells if interior is furnished.
                var tier2 = profiles
                    .Where(AllowedId)
                    .Where(p => !p.IsPairedHalf)
        ...
        public static HashSet<uint> GetEditorEnvCellFallbackModelIds(IDatReaderWriter dats) {
            lock (_lock) {
                var profiles = GetOrBuildProfiles(dats);
                return profiles
                    .Where(p => IsSetupModelId(p.ModelId))
        ...
        private static List<BuildingProfile> RunAnalysis(IDatReaderWriter dats, bool verbose) {
            ...
            var allLbiIds = dats.Cell().GetAllIdsOfType<LandBlockInfo>().ToArray();
