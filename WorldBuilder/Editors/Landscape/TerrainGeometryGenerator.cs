...
namespace WorldBuilder.Editors.Landscape {
    ...
    public static class TerrainGeometryGenerator {
        ...
        private static void GenerateCell(
            ...
            surfaceManager.FillVertexData(landblockID, cellX, cellY, baseLandblockX, baseLandblockY, ref v0, bottomLeft.Height, surfInfo, 0, rotation);
            surfaceManager.FillVertexData(landblockID, cellX + 1, cellY, baseLandblockX, baseLandblockY, ref v1, bottomRight.Height, surfInfo, 1, rotation);
            surfaceManager.FillVertexData(landblockID, cellX + 1, cellY + 1, baseLandblockX, baseLandblockY, ref v2, topRight.Height, surfInfo, 2, rotation);
            surfaceManager.FillVertexData(landblockID, cellX, cellY + 1, baseLandblockX, baseLandblockY, ref v3, topLeft.Height, surfInfo, 3, rotation);
        ...
        public static void GetCellRotation(LandSurfaceManager landSurf, uint landblockID, TerrainEntry[] terrain, uint x, uint y, ref uint surfNum, ref TextureMergeInfo.Rotation rotation) {
            ...
            // Outdoor TexMerge path: build a single full corner palette code and
            // keep the base rotation at Rot0. The begin-rotation logic is for the PalShift path,
            // not normal TexMerge terrain selection.
            uint paletteCode = TerrainPalCode.GetPalCode(
                (int)r1, (int)r2, (int)r3, (int)r4, (int)t1, (int)t2, (int)t3, (int)t4,
                landSurf.TextureSizeBits);
            rotation = TextureMergeInfo.Rotation.Rot0;
            landSurf.SelectTerrain(globalCellX, globalCellY, out surfNum, out _, paletteCode);
        }

        public static uint GetPalCode(int r1, int r2, int r3, int r4, int t1, int t2, int t3, int t4) {
            return TerrainPalCode.GetPalCode(r1, r2, r3, r4, t1, t2, t3, t4, TerrainPalCode.GetTextureSizeBits(isPalShifted: false));
        }
