    ...
    public partial class LandscapeEditorViewModel : ViewModelBase {
        ...
        public string CurrentToolHint {
            get {
                ...
                var sub = SelectedSubTool;
                if (tool is null)
                    return "Choose Select, Terrain, Road, or Height. Sub-tools appear next to the selected tool.";
                if (sub is null)
                    return string.IsNullOrWhiteSpace(tool.Description) ? tool.Name : tool.Description;
                if (sub is HeightRaiseLowerSubToolViewModel)
                    return "Drag on the ground to raise it. Hold Shift before starting a stroke to lower it. Brush radius controls the affected area.";
                if (sub is HeightSetSubToolViewModel)
                    return "Flatten the ground to a height. Alt-click to sample an existing height from the terrain. [ and ] resize the brush.";
                if (sub is HeightSmoothSubToolViewModel)
                    return "Drag across uneven ground to soften changes in height. Use a smaller brush for edges and a larger brush for slopes.";
                if (sub is BrushSubToolViewModel)
                    return "Choose a texture in the palette, adjust the brush radius, then drag on the ground to paint. Undo reverses your last stroke.";
                var detail = string.IsNullOrWhiteSpace(tool.Description) ? tool.Name : tool.Description;
                return $"{tool.Name} → {sub.Name}. {detail}";
            }
        }
        ...
        /// <summary>Current project (for outdoor instance placements).</summary>
        public Project? Project => _project;
        public bool IsReadOnlyProject => _project?.IsReadOnlyDatProject == true;
        ...
        internal void Init(Project project) {
            ...
            _project = project;
            OnPropertyChanged(nameof(IsReadOnlyProject));
        ...
        private void InitDocking() {
            ...
            var preferredDock = new Dictionary<string, DockLocation>(StringComparer.OrdinalIgnoreCase) {
                ["ObjectBrowser"] = DockLocation.Left,
                ["TexturePalette"] = DockLocation.Left,
                ["Toolbox"] = DockLocation.Right,
                ["Layers"] = DockLocation.Right,
                ["History"] = DockLocation.Right,
                ["Bookmarks"] = DockLocation.Right,
                ["WorldMap"] = DockLocation.Right
            };
            var toolDrivenLeft = new HashSet<string>(StringComparer.OrdinalIgnoreCase) {
                "ObjectBrowser", "TexturePalette"
            };
            ...
            foreach (var layout in layouts) {
                ...
                if (Enum.TryParse<DockLocation>(layout.Location, out var savedLoc)
                    && savedLoc != expectedLoc
                    && (savedLoc == DockLocation.Bottom
                        || savedLoc == DockLocation.Top
                        || savedLoc == DockLocation.Center)) {
                    layout.Location = expectedLoc.ToString();
                }
            }

            void Register(string id, string title, object content, DockLocation defaultLoc, bool defaultVisible = true) {
            ...
            Register("Toolbox", "Options", new ToolboxViewModel(this), DockLocation.Right);
            if (ObjectBrowser != null) Register("ObjectBrowser", "Objects", ObjectBrowser, DockLocation.Left);
            if (TexturePalette != null) Register("TexturePalette", "Textures", TexturePalette, DockLocation.Left, defaultVisible: false);
            if (LayersPanel != null) Register("Layers", "Layers", LayersPanel, DockLocation.Right);
            if (HistorySnapshotPanel != null) Register("History", "History", HistorySnapshotPanel, DockLocation.Right, defaultVisible: false);
            if (BookmarksPanel != null) Register("Bookmarks", "Bookmarks", BookmarksPanel, DockLocation.Right);
            if (WorldMapPanel != null) Register("WorldMap", "Map", WorldMapPanel, DockLocation.Right);
            ...
            foreach (var panel in DockingManager.AllPanels) {
                ...
                if (panel.Location == DockLocation.Bottom
                    || panel.Location == DockLocation.Top
        ...
        private void HandleViewportKeyDown(KeyEventArgs e) {
            ...
            switch (e.Key) {
                case Avalonia.Input.Key.F:
                    FocusSelection();
                    break;
                case Avalonia.Input.Key.OemOpenBrackets:
                    AdjustActiveBrushRadius(-1f);
                    break;
                case Avalonia.Input.Key.OemCloseBrackets:
                    AdjustActiveBrushRadius(1f);
                    break;
            ...
            if (hit.HasValue && hit.Value.Hit) {
                ...
                if (entries != null) {
                    ...
                    if (vi >= 0 && vi < entries.Length) {
                        ...
                        var terrainType = (TerrainTextureType)typeByte;
        ...
        private void UpdateLeftPanel() {
            bool showTextures = SelectedTool is TexturePaintingToolViewModel;
            bool showObjects = SelectedTool is SelectorToolViewModel;
            ...
            SetPanelVisible("TexturePalette", showTextures);
            SetPanelVisible("ObjectBrowser", showObjects);

            LeftPanelContent = showTextures ? TexturePalette : showObjects ? ObjectBrowser : null;
            LeftPanelTitle = showTextures ? "Textures" : showObjects ? "Objects" : "";
        }
        ...
        private void OnPaletteTextureSelected(object? sender, TerrainTextureType type) {
        ...
        private async Task FreshStart() {
            if (TerrainSystem == null || IsReadOnlyProject) return;
        ...
        private async Task GenerateWorld() {
            if (TerrainSystem == null || _dats == null || IsReadOnlyProject) return;
        ...
        /// <summary>
        /// Outdoor teleport format: full id = (landblockKey * 65536) | outdoorCellId,
        ...
        private async Task<bool> ShowFreshStartConfirmation() {
            ...
            await DialogHost.Show(new StackPanel {
                ...
                Children = {
                    ...
                    new TextBlock {
                        Text = "Note: Server-side spawns in your world database\nwill not be removed.\nYou may need to clear those rows separately.",
                        TextWrapping = TextWrapping.Wrap,
                        MaxWidth = 400,
                        FontSize = 11,
                        Opacity = 0.6,
        ...
        private async Task LoadWeenieSpawnsForLandblockAsync(ushort lbKey) {
            ...
            try {
                ...
                if (aceConn == null || string.IsNullOrWhiteSpace(aceConn.Host)) {
                    Console.WriteLine($"[Spawns] No world database configured — cannot load weenie spawns");
                    return;
                }
