...
namespace WorldBuilder.Editors.Landscape {
    ...
    public class ThumbnailRenderService : IDisposable {
        ...
        // Thumbnails are for identification, so prefer color-faithful unlit previews over dramatic shading.
        private const float AmbientIntensity = 1.0f;
        private const float SpecularPower = 0f;
        ...
        private unsafe byte[]? RenderThumbnail(uint id, bool isSetup) {
            ...
            _objectManager._objectShader.SetUniform("uHighlightColor", Vector3.Zero);
            _objectManager._objectShader.SetUniform("uHighlightIntensity", 0f);
            _objectManager._objectShader.SetUniform("uFogEnabled", 0);
            _objectManager._objectShader.SetUniform("uFogColor", Vector3.Zero);
            _objectManager._objectShader.SetUniform("uFogStart", 0f);
            _objectManager._objectShader.SetUniform("uFogEnd", 1f);
