...
namespace WorldBuilder.Editors.Landscape {
    ...
    public class EnvCellManager : IDisposable {
        ...
        private readonly Dictionary<uint, Acme.Dat.Environment> _environmentCache = new();
        ...
        // CPU mesh keyed by GPU identity so dungeon hover/refresh does not rebuild the same room.
        private readonly Dictionary<EnvCellGpuKey, PreparedCellStructMesh> _cpuMeshCache = new();
        private const int MaxCpuMeshCacheEntries = 256;

        // Decoded surface textures reused across CellStructs (many polygons share one Surface).
        private readonly object _prepCacheLock = new();
        private readonly Dictionary<(uint SurfaceId, StipplingType Stippling, bool IsSolid), TextureLoadCacheEntry> _textureLoadCache = new();
        private readonly Dictionary<uint, Surface> _surfaceCache = new();
        private readonly HashSet<uint> _failedSurfaces = new();
        ...
        /// <summary>
        /// When true (default), only draw cells reachable through portals from the camera cell.
        /// The dungeon editor turns this off so neighboring rooms stay visible through a
        /// doorway even if the frames are a few units off — otherwise the opening looks like
        /// a black void until you walk through it.
        /// </summary>
        ...
        private readonly ConcurrentQueue<(ushort Key, bool ReleaseGpu)> _pendingUnloads = new();
        ...
        /// </summary>
        public int LoadedDungeonCellCount {
            get {
                lock (_cellLock) {
                    int n = 0;
                    foreach (var kvp in _loadedCells) {
                        if (_dungeonOnlyLandblocks.Contains(kvp.Key)) {
                            n += kvp.Value.Count;
                        }
                        else if (_mixedLandblocks.Contains(kvp.Key)) {
                            foreach (var cell in kvp.Value) {
                                if (!cell.IsBuilding) n++;
                            }
                        }
                    }
                    return n;
        ...
        /// </summary>
        /// <summary>
        /// Underground dungeon geometry is pushed well below the
        ...
        private PreparedEnvCell? PrepareEnvCell(EnvCell envCell, Vector3 lbOffset, ushort lbKey = 0, float dungeonZBump = 0f) {
            ...
            Acme.Dat.Environment? env;
            lock (_environmentCache) {
                if (!_environmentCache.TryGetValue(envFileId, out env)) {
                    if (!_dats.TryGet<Acme.Dat.Environment>(envFileId, out env)) {
            ...
            if (!env.Cells.TryGetValue((uint)envCell.CellStructure, out var cellStruct)) {
                return null;
            }
            ...
            PreparedCellStructMesh? meshData;
            lock (_cellLock) {
                _cpuMeshCache.TryGetValue(gpuKey, out meshData);
            }
            if (meshData == null) {
                meshData = PrepareCellStructMesh(cellStruct, surfaceIds, portalPolyIds);
                if (meshData == null) return null;
                lock (_cellLock) {
                    if (_cpuMeshCache.Count >= MaxCpuMeshCacheEntries) {
                        _cpuMeshCache.Clear();
                    }
                    _cpuMeshCache[gpuKey] = meshData;
                }
            }
        ...
        private PreparedCellStructMesh? PrepareCellStructMesh(CellStruct cellStruct, List<uint> surfaceIds, HashSet<ushort>? portalPolygonIds = null) {
            ...
            foreach (var kvp in cellStruct.Polygons) {
                ...
                Surface? surface;
                bool isSolid = false;
                (byte[] Data, int Width, int Height, TextureFormat Format, PixelFormat? UploadPixelFormat, PixelType? UploadPixelType, uint PaletteId)? texResult = null;
                bool loadTexture = false;

                lock (_prepCacheLock) {
                    if (_failedSurfaces.Contains(surfaceId)) {
                        surface = null;
                    }
                    else if (!_surfaceCache.TryGetValue(surfaceId, out surface)) {
                        if (_dats.TryGet<Surface>(surfaceId, out surface) && surface != null) {
                            _surfaceCache[surfaceId] = surface;
                        }
                        else {
                            _failedSurfaces.Add(surfaceId);
                            surface = null;
                        }
                    }
                    ...
                    if (surface != null) {
                        ...
                        var loadKey = (surfaceId, poly.Stippling, isSolid);
                        if (_textureLoadCache.TryGetValue(loadKey, out var cachedTex)) {
                            isSolid = cachedTex.IsSolid;
                            texResult = cachedTex.TexResult;
                        }
                        else {
                            loadTexture = true;
                        }
                ...
                if (loadTexture && surface != null) {
                    texResult = LoadTextureData(surfaceId, surface, isSolid, poly.Stippling);
                    lock (_prepCacheLock) {
                        _textureLoadCache[(surfaceId, poly.Stippling, isSolid)] = new TextureLoadCacheEntry {
                            IsSolid = isSolid,
                            TexResult = texResult
                        };
                    }
                }
                else if (surface == null && CustomTextureResolver?.Invoke(surfaceId) is { } customTex) {
                    isSolid = false;
        ...
        private (byte[] data, int width, int height, TextureFormat format, PixelFormat? uploadFormat, PixelType? uploadType, uint paletteId)?
            ...
            if (TextureHelpers.IsCompressedFormat(renderSurface.FormatEnum)) {
                var fmt = renderSurface.FormatEnum switch {
                    DatPixelFormat.PFID_DXT1 => TextureFormat.DXT1,
                    DatPixelFormat.PFID_DXT3 => TextureFormat.DXT3,
                    DatPixelFormat.PFID_DXT5 => TextureFormat.DXT5,
            ...
            switch (renderSurface.FormatEnum) {
                ...
                case DatPixelFormat.PFID_INDEX16: {
                    ...
                    bool expand256Palette = !WorldBuilder.Shared.Lib.LegacyDatReader.IsSyntheticRenderSurfaceId(renderSurface.Id);
        ...
        /// </summary>
        public void QueueUnload(ushort lbKey) => QueueUnload(lbKey, releaseUnreferencedGpu: true);

        /// <summary>
        /// Queues a landblock for deferred unload on the GL thread.
        /// Safe to call from any thread. The actual GL resource deletion
        /// happens during <see cref="ProcessUploads"/> on the render thread.
        /// Pass <paramref name="releaseUnreferencedGpu"/> false when the next upload
        /// will reuse the same room meshes (dungeon refresh / placement ghost).
        /// </summary>
        public void QueueUnload(ushort lbKey, bool releaseUnreferencedGpu) {
            _pendingUnloads.Enqueue((lbKey, releaseUnreferencedGpu));
        }
        ...
        public int ProcessUploads(int maxPerFrame = 4) {
            while (_pendingUnloads.TryDequeue(out var pending)) {
                UnloadLandblock(pending.Key, pending.ReleaseGpu);
            }
        ...
        private unsafe void FinalizeGpuUpload(PreparedEnvCellBatch batch) {
            ...
            foreach (var prepared in batch.Cells) {
                ...
                if (renderData == null) {
                    if (prepared.MeshData == null) continue;
            ...
            if (cells.Count > 0) {
                lock (_cellLock) {
                    if (batch.Append && _loadedCells.TryGetValue(batch.LandblockKey, out var existing))
                        existing.AddRange(cells);
                    else
        ...
        public unsafe void Render(Matrix4x4 viewProjection, ICamera camera, Vector3 lightDirection, float ambientIntensity, float specularPower) {
            ...
            // otherwise compute it now (backwards-compat). Skip when portal culling
            // is off so the dungeon editor can look straight through doorways.
            var visibility = UsePortalCulling
            ...
            foreach (var kvp in _loadedCells) {
                ...
                bool usePortalFilter =
                    UsePortalCulling && !AlwaysShowBuildingInteriors && visibility != null && kvp.Key == cameraLbKey;
                ...
                foreach (var cell in kvp.Value) {
                    ...
                    if (isPreview) {
                        ...
                        if (!frustum.IntersectsBoundingBox(cell.GetWorldAabb())) continue;
                    }
        ...
        /// </summary>
        public void UnloadLandblock(ushort lbKey) => UnloadLandblock(lbKey, releaseUnreferencedGpu: true);

        /// <summary>
        /// Unloads all dungeon cells for a landblock.
        /// Must be called on the GL thread.
        /// </summary>
        public void UnloadLandblock(ushort lbKey, bool releaseUnreferencedGpu) {
        ...
        /// <summary>
        /// Move already-loaded cells (placement ghost) without rebuilding meshes.
        /// Returns false if the landblock is missing or room identity no longer matches.
        /// </summary>
        public bool TryPatchLandblockTransforms(ushort lbKey, uint lbId, List<EnvCell> envCells, bool isDungeonOnly) {
            ...
            lock (_cellLock) {
                if (!_loadedCells.TryGetValue(lbKey, out var loaded) || loaded.Count != envCells.Count) {
                    return false;
                }

                for (int i = 0; i < envCells.Count; i++) {
                    uint envFileId = envCells[i].EnvironmentId | 0x0D000000u;
                    if (loaded[i].EnvironmentId != envFileId
                        || loaded[i].GpuKey.CellStructure != envCells[i].CellStructure
                        || loaded[i].SurfaceCount != envCells[i].Surfaces.Count) {
                        return false;
                    }
                }

                for (int i = 0; i < envCells.Count; i++) {
                    var envCell = envCells[i];
                    bool isBuildingCell = !isDungeonOnly && envCell.Flags.HasFlag(EnvCellFlags.SeenOutside);
                    float cellZBump = isBuildingCell
                        ? BuildingFloorOffset
                        : (isDungeonOnly ? DungeonDepthOffset : 0f);
                    var cellOrigin = envCell.Position.Origin + lbOffset;
                    cellOrigin.Z += cellZBump;
                    var worldTransform = Matrix4x4.CreateFromQuaternion(envCell.Position.Orientation)
                        * Matrix4x4.CreateTranslation(cellOrigin);
                    Matrix4x4.Invert(worldTransform, out var inverseTransform);
                    loaded[i].WorldTransform = worldTransform;
                    loaded[i].WorldPosition = cellOrigin;
                    loaded[i].InverseWorldTransform = inverseTransform;
                }
            }
        ...
        public VisibilityResult? GetVisibleCells(Vector3 cameraPos, Frustum frustum) {
            ...
            while (queue.Count > 0) {
                ...
                for (int i = 0; i < cell.Portals.Count; i++) {
                    ...
                    if (!frustum.IntersectsBoundingBox(neighbor.GetWorldAabb())) continue;
        ...
        public void ClearAll() {
            ...
            _cpuMeshCache.Clear();
            _textureLoadCache.Clear();
            _surfaceCache.Clear();
            _failedSurfaces.Clear();
            _loadedCells.Clear();
        ...
        private sealed class TextureLoadCacheEntry {
            public bool IsSolid;
            public (byte[] Data, int Width, int Height, TextureFormat Format, PixelFormat? UploadPixelFormat, PixelType? UploadPixelType, uint PaletteId)? TexResult;
        }
    }
    ...
    public class LoadedEnvCell {
        ...
        /// <summary>Editor-only: skip drawing this cell in the dungeon viewport.</summary>
        public bool IsHidden { get; set; }

        /// <summary>
        /// World-space AABB from the cell's local mesh bounds. Falls back to a large
        /// radius when bounds were never computed.
        /// </summary>
        public BoundingBox GetWorldAabb(float pad = 0.75f) {
            if (LocalBoundsMin.X >= LocalBoundsMax.X) {
                var r = new Vector3(EnvCellManager.CellBoundsRadius);
                return new BoundingBox(WorldPosition - r, WorldPosition + r);
            }

            var min = new Vector3(float.MaxValue);
            var max = new Vector3(float.MinValue);
            var lo = LocalBoundsMin;
            var hi = LocalBoundsMax;
            for (int i = 0; i < 8; i++) {
                var local = new Vector3(
                    (i & 1) == 0 ? lo.X : hi.X,
                    (i & 2) == 0 ? lo.Y : hi.Y,
                    (i & 4) == 0 ? lo.Z : hi.Z);
                var world = Vector3.Transform(local, WorldTransform);
                min = Vector3.Min(min, world);
                max = Vector3.Max(max, world);
            }
            var p = new Vector3(pad);
            return new BoundingBox(min - p, max + p);
        }
    }
    ...
    public class PreparedEnvCellBatch {
        ...
        /// <summary>When true, GPU upload merges these cells into an already-loaded landblock.</summary>
