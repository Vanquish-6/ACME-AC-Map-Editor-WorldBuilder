...
namespace WorldBuilder.Editors.Monster {
    ...
    public partial class MonsterEditorViewModel : ViewModelBase {
        ...
        // Creature picker
        [ObservableProperty] private string _statusText = "Configure the world database in Settings, then search for a base creature.";
        ...
        [ObservableProperty] private ColorARGB[]? _previewCreaturePalette;
        ...
        private async Task SearchWeeniesAsync() {
            if (Settings?.AceDbConnection == null) { StatusText = "Configure the world database in Settings first."; return; }
        ...
        private async Task SearchDonorWeeniesAsync() {
            if (Settings?.AceDbConnection == null) { DonorPartsStatus = "Configure the world database in Settings first."; return; }
            IsDonorBusy = true;
            DonorWeenies.Clear();
            ...
            if (overrides.ClothingBase != 0 &&
                ...
                foreach (var kvp in clothingTable.ClothingSubPalEffects) {
                    foreach (var subPalette in kvp.Value.CloSubPalettes) {
                        uint palSetId = subPalette.PaletteSet;
                        ...
                        uint subPalDid = palSet.Palettes[templateIdx];
                        ...
                        foreach (var range in subPalette.Ranges) {
                            ...
                            for (int i = 0; i < numColors; i++) {
                                ...
                                var src = ColorARGB.FromPacked(subPal.Colors[i]);
        ...
        async Task LoadBrowserItemsAsync() {
            ...
            var ids = await Task.Run(() => {
                try {
                    return dats.GetAllIdsOfType<SurfaceTexture>().OrderBy(id => id).ToArray();
                }
