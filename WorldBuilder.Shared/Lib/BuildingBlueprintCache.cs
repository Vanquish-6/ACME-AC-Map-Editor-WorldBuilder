    ...
    public static class BuildingBlueprintCache {
        ...
        private static void EnsureBuildingIdsSscanned(IDatReaderWriter dats) {
            ...
            lock (_scanLock) {
                ...
                var allLbiIds = dats.GetAllIdsOfType<LandBlockInfo>().ToArray();
        ...
        private static BuildingBlueprint? ExtractFromDonor(BuildingInfo donor, uint donorLbId, IDatReaderWriter dats, ILogger? logger) {
            ...
            foreach (var cellId in cellIds.OrderBy(c => c)) {
                ...
                // Copy surfaces
                snapshot.Surfaces.AddRange(envCell.Surfaces.Select(s => (ushort)s));
                ...
                // find_transit_cells portal lookups, resulting in walk-through walls.
        ...
        public static (BuildingInfo building, int cellCount)? InstantiateBlueprint(
            ...
            foreach (var cell in blueprint.Cells) {
                ...
                // Copy surfaces
                envCell.Surfaces.AddRange(cell.Surfaces.Select(s => (uint)s));
                ...
                // different position than the donor, these stale references cause
                ...
                logger?.LogInformation("[Blueprint]   FRAME donor origin=({X},{Y},{Z}) quat=({W},{QX},{QY},{QZ})",
                ...
                logger?.LogInformation("[Blueprint]   FRAME new origin=({X},{Y},{Z}) quat=({W},{QX},{QY},{QZ})",
                    envCell.Position.Origin.X, envCell.Position.Origin.Y, envCell.Position.Origin.Z,
                ...
                try {
                    int donorSize = DatNativeRecords.Pack(donorCell).Length;
                    int newSize = DatNativeRecords.Pack(envCell).Length;
                    if (donorSize != newSize) {
                        logger?.LogWarning("[Blueprint]   FULL SIZE MISMATCH: donor={D} vs new={N}", donorSize, newSize);
                    }
                } catch (Exception packEx) {
                    logger?.LogWarning("[Blueprint]   Pack diag failed: {Msg}", packEx.Message);
                }
                    }
                } catch (Exception ex) {
