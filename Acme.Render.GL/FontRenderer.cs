...
namespace Acme.Render.GL {
    public class Texture2DManager : ITexture2DManager {
        ...
        public object CreateTexture(int width, int height) {
            var texture = _renderer.GraphicsDevice.CreateTexture(Acme.Render.Enums.TextureFormat.RGBA8, width, height, new byte[width * height * 4]);

            return texture;
        }

        public Point GetTextureSize(object texture) {
            var t = (Acme.Render.ITexture)texture;
        ...
        public void SetTextureData(object texture, Rectangle bounds, byte[] data) {
            ...
            t.SetData(new Acme.Render.Rectangle(bounds.X, bounds.Y, bounds.Width, bounds.Height), data);
        }
    }
    ...
    internal class FontRenderer : IFontRenderer, IDisposable {
        ...
        public unsafe FontRenderer(OpenGLRenderer renderer) {
            ...
            _vertexBuffer = _renderer.GraphicsDevice.CreateVertexBuffer(MAX_VERTICES * sizeof(VertexPositionColorTexture), Acme.Render.Enums.BufferUsage.Dynamic);
            _indexBuffer = _renderer.GraphicsDevice.CreateIndexBuffer(MAX_INDICES * sizeof(uint), Acme.Render.Enums.BufferUsage.Dynamic);

            _indexBuffer.SetData(indexData);

            _vao = _renderer.GraphicsDevice.CreateArrayBuffer(_vertexBuffer, Chorizite.Acme.Render.Vertex.VertexPositionColorTexture.Format);
        }
        ...
        public unsafe void Flush() {
            ...
            _vertexBuffer.SetData(_vertexData.Select(v => {
                return new Chorizite.Acme.Render.Vertex.VertexPositionColorTexture(v.Position, new ColorVec(v.Color.R / 255f, v.Color.G / 255f, v.Color.B / 255f, v.Color.A / 255f), v.TextureCoordinate);
            }).ToArray());

            var texture = (Acme.Render.ITexture)_lastTexture;
            texture.Bind();

            _renderer.GraphicsDevice.DrawElements(Acme.Render.Enums.PrimitiveType.TriangleList, (_vertexIndex * 6 / 4), 0);
            _vertexIndex = 0;
            texture.Unbind();
        }
