...
This guide explains how to install the app, create a project, move between tools, and export safely.

---

## What this application is

ACME WorldBuilder is a desktop editor for **Asheron's Call** client data. It supports two DAT modes:

- **Retail DAT projects**: full editor workflow with export to updated `client_*.dat` files.
- **Legacy pre-ToD DAT projects**: read-only viewing for older `cell.dat` / `portal.dat` layouts.

Some tools also talk to a **MySQL world database** (weenies, creatures, export helpers).
...
- For **Weenie Editor**, **Monster Creator**, and some **export** options: a reachable **MySQL** server with your world's data, configured in **Settings**.
...
2. **Base DAT directory** — either:
   - a retail client folder with `client_cell_1.dat`, `client_portal.dat`, `client_highres.dat`, and `client_local_English.dat`, or
   - a legacy pre-ToD folder with `cell.dat` and `portal.dat`

The app detects the DAT mode automatically when you create the project.
...
If the project points at a **legacy pre-ToD** DAT set, the app opens with a clear **read-only** banner. Landscape viewing stays available, but export and retail-only editors are disabled.
...
| Edit **weenie** scalar properties in MySQL | **Weenie Editor** |
...
- **World database / MySQL** — host, port, database name, user, and password used by **Weenie Editor** and **Monster Creator** to list and save weenies. Use **Test Connection** where offered.
...
Connects to your **world database**.
...
> Complex weenie features (generators, nested spell structures, etc.) may still require SQL outside this editor. This editor targets **scalar property tables** clearly.

---

## Monster Creator

For **creature appearance**: which surface texture IDs each body part uses, optional **donor** creature parts, and **SQL** you can apply to your world database.
...
Export is only available for **retail** DAT projects. Legacy pre-ToD projects are intentionally view-only.

1. **File → Export Dats…**
2. Choose an **export directory** (often a staging copy of your client `dat` folder).
3. Set **Portal iteration** as required by your workflow.
4. Enable **Overwrite** only if you intend to replace files that are already there.

### Optional: world database helpers on export
...
If **Apply changes directly to database** is enabled, the app may run the generated SQL immediately (see success dialog text). Turn that off if you prefer to review files first.

These features assume you know how your shard applies SQL and how instances are keyed in the world database.
...
- **Updates:** desktop builds can check for published updates.
